/**
 * Honda Parser
 *
 * Parses spec pages from powersports.honda.com into structured text.
 * Honda pages use structured HTML with spec sections containing
 * label-value pairs in tables or definition lists.
 */

import type { ParsedSection } from '../scraper.types'

export async function parseHondaHtml(
  html: string,
  title: string
): Promise<{ fullText: string; sections: ParsedSection[] }> {
  const { JSDOM } = await import('jsdom')
  const dom = new JSDOM(html)
  const doc = dom.window.document

  const specs = extractSpecs(doc)
  const features = extractFeatures(doc)
  const sections: ParsedSection[] = []
  const parts: string[] = []

  parts.push(cleanTitle(title).toUpperCase())
  parts.push('')

  if (specs.length > 0) {
    const specContent = specs.map(({ label, value }) => `${label}: ${value}`).join('\n')
    parts.push('SPECIFICATIONS')
    parts.push(specContent)
    parts.push('')
    sections.push({ heading: 'Specifications', content: specContent, level: 2 })
  }

  if (features) {
    parts.push('FEATURES')
    parts.push(features)
    parts.push('')
    sections.push({ heading: 'Features', content: features, level: 2 })
  }

  return { fullText: parts.join('\n').trim(), sections }
}

interface SpecRow {
  label: string
  value: string
}

function extractSpecs(doc: Document): SpecRow[] {
  const rows: SpecRow[] = []

  // Honda uses tables with spec rows, or dl/dt/dd pairs
  const tables = doc.querySelectorAll('table')
  for (const table of Array.from(tables)) {
    for (const tr of Array.from(table.querySelectorAll('tr'))) {
      const cells = tr.querySelectorAll('td, th')
      if (cells.length >= 2) {
        const label = cleanText(cells[0].textContent || '')
        const value = cleanText(cells[1].textContent || '')
        if (label && value && isSpecLabel(label)) {
          rows.push({ label, value })
        }
      }
    }
  }

  // Also check definition lists
  const dls = doc.querySelectorAll('dl')
  for (const dl of Array.from(dls)) {
    const dts = dl.querySelectorAll('dt')
    const dds = dl.querySelectorAll('dd')
    for (let i = 0; i < Math.min(dts.length, dds.length); i++) {
      const label = cleanText(dts[i].textContent || '')
      const value = cleanText(dds[i].textContent || '')
      if (label && value) {
        rows.push({ label, value })
      }
    }
  }

  return rows
}

function extractFeatures(doc: Document): string | null {
  const paragraphs = doc.querySelectorAll('p, .feature-description, .overview')
  const parts: string[] = []
  for (const el of Array.from(paragraphs)) {
    const text = cleanText(el.textContent || '')
    if (text.length >= 40) parts.push(text)
  }
  return parts.length > 0 ? parts.join('\n\n') : null
}

function cleanText(text: string): string {
  return text.replace(/\s+/g, ' ').trim()
}

function cleanTitle(title: string): string {
  return title
    .replace(/\s*[-–|]\s*Honda\s*Powersports.*/i, '')
    .replace(/\s*[-–|]\s*Honda.*/i, '')
    .trim()
}

const SPEC_LABELS =
  /^(engine|displacement|bore|stroke|compression|power|torque|transmission|final drive|fuel|cooling|frame|suspension|brake|tire|tyre|weight|seat height|wheelbase|length|width|height|fuel capacity|ground clearance|trail|rake)/i

function isSpecLabel(label: string): boolean {
  return SPEC_LABELS.test(label) || label.length < 40
}
